...
/* Los cuatro tipos que se venden: el precio de lista de cada uno sale de textos.php. */
...
caso('el precio llega en DOS mensajes: la oferta con el link y, aparte, los tres pasos, SIN el formulario (Pablo, 14-sep)',
    count($r) === 2 && stripos($r[0], 'El primer paso es gratis') === false && stripos($r[1], 'El primer paso es gratis') !== false
...
caso('el primer mensaje trae lo que incluye y las dos formas con sus montos (Pablo, 15-sep)',
    ...
    && mb_stripos($r[0], '2. Servicio mensual de $20.000, sin pago inicial') !== false);
caso('el segundo mensaje ofrece la demo gratis en menos de 24 horas, sin montos (14-sep)',
    preg_match('/^El primer paso es gratis: te armamos una demo.*menos de 24 horas/u', $r[1]) === 1 && strpos($r[1], '$') === false);
...
caso('la demo se nombra en ese mismo mensaje',
    stripos($r[0], 'demo') !== false || stripos($r[0], 'muestra') !== false);

$c = conv_nueva();
clasifica(['pregunta_info'], ['info_keys' => ['pago']]);
$r = wabot_engine('se puede pagar en cuotas?', $c, $cfg);
/* 10-sep: el texto de pago ya termina preguntando a qué se dedica, así que el
 * menú no se suma atrás: serían dos preguntas iguales seguidas. */
caso('pregunta de info en el primer mensaje → responde con los dos pares y pregunta el rubro',
    $r[0] === wabot_texto_pago_generico($cfg) && (count($r) === 1 || $r[1] === $cfg['menu']) && $c['fase'] === 'menu');
/* 10-sep: el modelo nuevo no tiene seña. El primer pago es un número público
...
caso('nombra las dos formas de pagarla, sin montos (15-sep)', stripos($r[0], 'dos formas de pagarla') !== false);

echo "— pago_generico: los dos pares y cómo se paga cada uno (10-sep) —\n";

$pagoGenerico = wabot_texto_pago_generico($cfg);
caso('explica las dos formas de pagarla, sin montos antes de saber el rubro (15-sep)',
    stripos($pagoGenerico, 'dos formas de pagarla') !== false && strpos($pagoGenerico, '$') === false);
caso('y que el mensual es sin pago inicial', stripos($pagoGenerico, 'sin pago inicial') !== false);
caso('el pago único arranca con una seña, sin decir el monto', stripos($pagoGenerico, 'seña para arrancar') !== false);
caso('y el saldo va al entregar', stripos($pagoGenerico, 'saldo al entregar') !== false);
caso('y no deja ningún marcador crudo', strpos($pagoGenerico, '{') === false);

$cfgSenaNueva = wabot_config_load();
$cfgSenaNueva['tipos']['landing']['precio'] = '$65.000';
caso('sin rubro no hay montos, aunque cambie la lista de tipos (14-sep)',
    strpos(wabot_texto_pago_generico($cfgSenaNueva), '$65.000') === false);

$c = conv_nueva(); $c['fase'] = 'menu';
clasifica(['elige_ecommerce']);
$r = wabot_engine('ecommerce', $c, $cfg);
caso('elige ecommerce del menú → precio de la tienda online, sin link (14-sep)', strpos(implode("\n", (array)$r), '$30.000') !== false && strpos(implode("\n", (array)$r), '$30.000') !== false && strpos($r[0], 'Pago único de $290.000') !== false && strpos($r[0], 'presupuestos/') === false && $c['tipo'] === 'ecommerce');
...
caso('objeción caro → respuesta fija, cuotas y muestra, sin bajar precio',
    $r === [wabot_texto_caro($c, $cfg), $cfg['cta_muestra']] && $c['fase'] === 'precio' && $c['cta_muestra'] === true
    && strpos($r[0], '{') === false);

$c = conv_nueva(); $c['fase'] = 'precio'; $c['tipo'] = 'landing';
clasifica(['pregunta_info'], ['info_keys' => ['plazos', 'hosting']]);
$r = wabot_engine('en cuanto está y el hosting?', $c, $cfg);
caso('dos preguntas → una sola respuesta en bullets', count($r) === 1 && strpos($r[0], '- ') === 0 && strpos($r[0], '7 días') !== false && stripos($r[0], 'hosting') !== false);
...
caso('la objeción se contesta sin bajar el precio', $r === [wabot_texto_caro($c, $cfg)]);
...
caso('el turno A son DOS mensajes: la oferta con el servicio mensual y, aparte, la demo (14-sep)',
    count($rP) === 2 && strpos($rP[0], '$30.000') !== false && mb_stripos($rP[1], 'El primer paso es gratis') !== false
    && mb_stripos($rP[1], 'demo') !== false && mb_stripos($rP[0], 'El primer paso es gratis') === false);
...
caso('lo que cuenta después del precio no repite el precio ni vuelve a ofrecer la demo',
    strpos(implode(' ', $rP2), '$50.000') === false
...
foreach (['landing' => 'rubro_landing',
    ...
    caso("$tipoP también manda precio y demo en dos mensajes",
        ...
        && count($rx) === 2 && strpos($rx[0], '$') !== false && mb_stripos($rx[1], 'El primer paso es gratis') !== false);
...
foreach ($TIPOS as $tipoFijo) $textosFijosEsperados[$tipoFijo] = (string)$cfg['tipos'][$tipoFijo]['precio_ideal'];
caso('el texto de los cuatro tipos es el mismo: lo que cambia lo pone el código',
    count(array_unique($textosFijosEsperados)) === 1);
foreach ($textosFijosEsperados as $tipoFijo => $plantillaFija) {
    ...
    caso("$tipoFijo: el primer mensaje dice las dos formas con sus montos (15-sep)",
        ...
        && preg_match('/2\. Servicio mensual de \$[\d.]+, sin pago inicial/u', $rFijo[0]) === 1);
    ...
    caso("$tipoFijo: los tres pasos van en su propio mensaje, detrás del precio (14-sep)",
        count($rFijo) === 2 && mb_stripos($rFijo[1], 'El primer paso es gratis') === 0);
}
...
caso('quiere vender online → tienda online: $290.000 en un pago único o $30.000 por mes',
    strpos(implode(' ', $r), '$30.000') !== false && strpos(implode(' ', $r), '$30.000') !== false
    && strpos(implode(' ', $r), 'Pago único de $290.000') !== false
    && $c['tipo'] === 'ecommerce');
...
caso('y no se le cuela el monto del otro plan', strpos($r[0], '$25.000') === false);
...
caso('el texto de mantenimiento presenta el mensual como una de las dos formas, con el pago único al lado (15-sep)',
    stripos($cfg['info']['mantenimiento'], 'una de las dos formas') !== false && stripos($cfg['info']['mantenimiento'], 'pago único') !== false);
...
caso('explica las dos formas de contratarla (15-sep)', stripos($r[0], 'elegís cómo contratarla') !== false);
caso('sin montos, porque todavía no cotizó', strpos($r[0], '$') === false);
caso('con la demo gratis primero y el mensual sin pago inicial (15-sep)',
    stripos($r[0], 'demo gratis') !== false && stripos($r[0], 'servicio mensual, sin pago inicial') !== false);
caso('y la web lista en unos 7 días',
    stripos($r[0], 'queda lista en unos 7 días') !== false);
...
caso('preguntar cómo se paga explica las dos formas, sin montos antes de cotizar (15-sep)',
    stripos($r[0], 'dos formas de pagarla') !== false && strpos($r[0], '$') === false);
...
caso('las dos preguntas juntas → las dos respuestas en bullets',
    ...
    && stripos($r[0], 'Mercado Pago') !== false);
caso('y ninguna de las dos dice montos sin cotizar', strpos($r[0], '$') === false);
...
caso('decir que es caro sí dispara la respuesta oficial', $r[0] === wabot_texto_caro($c, $cfg));
...
/* 10-sep: el formulario ya NO sale con el precio (el segundo mensaje son los
 * tres pasos). El "Ok dale" es el sí, y recibe el formulario: ni la línea de
 * espera ni una derivación. */
...
$cLead['tipo'] = 'ecommerce';
$cLead['productos_cantidad'] = 40;
wabot_precio_congelar($cLead, 'ecommerce', $cfg);
...
caso('y el precio cotizado también, con las dos formas', preg_match('/^Pago único \$290\.000 \(seña \$60\.000\) o \$30\.000 por mes$/u', (string)$valor('presupuesto_cotizado')) === 1, (string)$valor('presupuesto_cotizado'));
...
caso('el objetivo junta lo dicho + la referencia',
...
caso('la etiqueta del tipo es la de la tienda', $valor('tipoDetectadoLabel') === 'Ecommerce');
...
caso('la repregunta NO es la pregunta original repetida', $r1[0] !== $cfg['desempate_hibrido']);
caso('es la versión simplificada, con las dos palabras esperadas',
    $r1[0] === $cfg['desempate_hibrido_2'] && strpos($r1[0], '"trabajos"') !== false && strpos($r1[0], '"vender"') !== false, $r1[0]);
...
caso('las dos primeras fueron la reformulación (nunca la original repetida)',
    $textos[0] === $cfg['desempate_hibrido_2'] && $textos[1] === $cfg['desempate_hibrido_2']);
...
caso('objeción en prediseno_ref → tampoco cierra',
    $r === [wabot_texto_caro($c, $cfg), $cfg['prediseno_referencia']] && $c['fase'] === 'prediseno_ref');
...
caso('sin nada que rescatar, primero reformula la pregunta y mantiene viva la charla',
    $r === [$cfg['desempate_hibrido_2']] && $c['fase'] === 'desempate_hibrido' && empty($c['handoff_pendiente']));
...
caso('sin cotizar dice las dos mensualidades de la lista, el pago único como alternativa, y pregunta el rubro (15-sep)',
    strpos($r[0], '$20.000 por mes en sitio profesional') !== false && strpos($r[0], '$30.000 en tienda online') !== false
    && stripos($r[0], 'pago único') !== false && strpos($r[0], '{') === false
...
// El precio con sus pasos y el formulario posterior tienen distinto largo.
// Sin techo, para no comparar dos demoras recortadas al mismo máximo.
$c = conv_nueva();
clasifica(['rubro_ecommerce']);
$r = wabot_engine('vendo mates', $c, $cfg);
$formDemora = wabot_prediseno_texto($c, $cfg);
$sinTecho = array_merge($t, ['demora_maxima' => 999]);
$d0Real = wabot_demora_tipeo($r[0], $sinTecho);
$d1Real = wabot_demora_tipeo($formDemora, $sinTecho);
caso('el precio y el formulario no tardan lo mismo, porque no miden lo mismo', $d0Real !== $d1Real);
$d0 = wabot_demora_tipeo($r[0], $t);
$d1 = wabot_demora_tipeo($formDemora, $t);
...
$ahoraSeg = time();
$vieja = conv_nueva();
$vieja['nombre'] = 'Marcos Pérez'; $vieja['nombre_confirmado'] = true;
$vieja['fase'] = 'derivado';
 ...
 * diariamente." Y desde el 15-sep el pago son las dos formas: pago único con
 * seña y saldo, o servicio mensual sin pago inicial. */
foreach ($TIPOS as $tipoCuota) {
    caso("$tipoCuota no tiene cuotas de lista en la config", empty($cfg['tipos'][$tipoCuota]['cuotas']));
...
caso('la charla cotizada entre el 10 y el 14-sep (snapshot mensual) conserva SU mensualidad y suma el pago único de lista',
    strpos(wabot_texto_pago($convCotizada, $cfg), 'Servicio mensual de $15.000') !== false
    && stripos(wabot_texto_pago($convCotizada, $cfg), 'Pago único de $180.000') !== false, wabot_texto_pago($convCotizada, $cfg));
...
caso('"cómo se paga" antes de tener precio explica la suscripción (14-sep)',
    stripos(wabot_texto_pago(['tipo' => 'ecommerce'], $cfg), 'servicio mensual por Mercado Pago') !== false);
...
/* Lo tiene que hablar con su socio, o ya tiene una web: lo sigue el
 * desarrollador directamente (Pablo, 15-sep). */
...
caso('"socio" deriva al desarrollador', $rs4 === [$cfg['derivar']] && $c4['fase'] === 'derivado' && !empty($c4['handoff_pendiente']), json_encode($rs4, JSON_UNESCAPED_UNICODE));
...
caso('"ya tiene web" también deriva, y antes le contesta lo de la web que ya tiene',
    count($rw5) === 2 && $rw5[0] === wabot_texto_info('ya_tiene_plataforma', $cfg, $c5) && end($rw5) === $cfg['derivar']
    && $c5['fase'] === 'derivado', json_encode($rw5, JSON_UNESCAPED_UNICODE));
...
caso('hosting: explica la renovación anual del pago único y no repite la oferta de demo',
    count($r) === 1
...
caso('el primer regateo recibe la respuesta oficial de precio, sin bajar el monto',
    $r === [wabot_texto_caro($c, $cfg)] && $c['fase'] === 'precio');
...
caso('"cual era el precio total" repite el TOTAL, no las cuotas solas',
    count($r) === 1 && strpos(implode("\n", (array)$r), '$180.000') !== false && strpos($r[0], '$32.000') === false);
...
caso('la tienda aclara que Gokywebs carga los primeros 10 productos al arrancar',
    stripos($cfg['info']['carga'], 'primeros 10 productos') !== false);
caso('y que el resto lo carga el cliente desde el panel, o nosotros a $500 por producto (10-sep)',
...
caso('aunque cambie la lista de tipos, sin rubro no se dicen montos',
...
foreach ($TIPOS as $tipo) {
    ...
    caso("$tipo no tiene un pagos3 calculado en la config: ese cálculo se retiró",
        !isset($cfg['tipos'][$tipo]['pagos3']));
}
...
caso('info.exclusividad existe y dice que es a medida',
...
/* 10-sep: el segundo mensaje son los tres pasos, y el link sale con el sí.
 * Quien pidió la demo al entrar ya dijo que sí: el formulario va en el mismo
 * turno, como tercer mensaje, sin re-ofrecer. */
caso('el precio, los tres pasos aparte y, como ya pidió la demo, el link del form atrás',
    count($rDemo) === 3 && mb_stripos($rDemo[1], 'El primer paso es gratis') !== false
    && strpos($rDemo[2], 'gokywebs.com/form/') !== false && mb_stripos($rDemo[1], 'Querés que') === false);
caso('y el del formulario es su propio mensaje, sin los pasos repetidos',
    mb_stripos($rDemo[2], 'formulario') !== false && mb_stripos($rDemo[2], 'El primer paso es gratis') === false);
...
caso('sin ese pedido, dos mensajes: el precio y los tres pasos, sin el formulario ni el listado de datos',
    count($rNormal) === 2 && mb_stripos($rNormal[1], 'El primer paso es gratis') !== false
...
caso('las descripciones no cuelgan un ejemplo del final del beneficio', (function () use ($cfg) {
    foreach (['ecommerce' => 'de madrugada', 'elearning' => 'links ni videos a mano'] as $tipo => $ejemplo) {
...
caso('el mensual es una de las dos formas, con su monto', mb_stripos($mant, 'una de las dos formas') !== false && strpos($mant, '$20.000 por mes') !== false);
caso('incluye hosting, dominio, soporte y mantenimiento técnico', mb_stripos($mant, 'mantenimiento técnico') !== false);
caso('sin permanencia', mb_stripos($mant, 'No hay permanencia') !== false);
caso('y ofrece el pago único como alternativa, sin inventar otro monto',
    mb_stripos($mant, 'pago único') !== false && substr_count($mant, '$') === 1);
...
caso('y presenta el mensual como una de las dos formas', stripos($mantSinTipo, 'una de las dos formas') !== false);
...
caso('y no manda a suscribirse antes de tiempo: sin el link de ninguno de los planes',
    strpos($mantSinTipo, $cfg['mantenimiento_planes']['otros']['link']) === false
    && strpos($mantSinTipo, $cfg['mantenimiento_planes']['landing']['link']) === false);
...
caso('el cierre de la recolección pide material como opcional',
    mb_stripos($cierreEcom, 'mandámelos') !== false && mb_stripos($cierreEcom, 'podemos empezar igual') !== false);
...
$cierreSinFotos = wabot_texto_prediseno_completo(['tipo' => 'ecommerce', 'imagenes_recibidas' => 0], $cfg);
caso('el cierre pide el material opcional', mb_stripos($cierreSinFotos, 'mandámelos') !== false);
...
caso('explica POR QUÉ lo pide, no lo pide a secas',
    mb_stripos($cierreSinFotos, 'para personalizarla') !== false);

$cierreConFotos = wabot_texto_prediseno_completo(['tipo' => 'ecommerce', 'imagenes_recibidas' => 3], $cfg);
...
caso('presentar manda un solo mensaje: la demo, sin el aviso de vigencia ni el pedido de feedback aparte',
    count($textosPresentar) === 1);
...
caso('si preguntan hasta cuándo dura, hay respuesta: 5 días (Pablo, 10-sep)',
    trim((string)($cfg['info']['demo_vigencia'] ?? '')) !== ''
    && stripos((string)$cfg['info']['demo_vigencia'], '5 días') !== false);
...
caso('confirmacion_demo_48h se manda sin parametros de cuerpo',
    (array)($cfgSinParams['plantillas']['confirmacion_demo_48h']['params'] ?? []) === []);
caso('confirmacion_demo_48h se manda sin parametros de boton',
    (array)($cfgSinParams['plantillas']['confirmacion_demo_48h']['boton'] ?? []) === []);
...
$rMixPrecio = wabot_precio('landing', $cMixPrecio, $cfg);
caso('cotizar un negocio mixto avisa en vez de cotizar',
    count($rMixPrecio) === 2 && stripos($rMixPrecio[0], 'integre') !== false, json_encode($rMixPrecio, JSON_UNESCAPED_UNICODE));
caso('y pregunta si lo quiere todo junto o arranca por una parte',
    $rMixPrecio[1] === (string)$cfg['mixto_pregunta']);
caso('sin dar el precio del sitio profesional como si cubriera todo',
    strpos($rMixPrecio[0], (string)$cfg['tipos']['landing']['precio']) === false
...
foreach ($TIPOS as $tipo) {
    $msg = wabot_msg_precio_texto($tipo, $cfg);
    caso("el precio de $tipo no linkea ni el presupuesto ni el portfolio (14-sep)",
        strpos($msg, (string)$cfg['tipos'][$tipo]['link']) === false
...
caso('ninguna variante del precio lleva el link del presupuesto (14-sep)',
...
foreach ($TIPOS as $tipo) {
    ...
    caso("el precio de $tipo no linkea el presupuesto ni el portfolio (14-sep)",
...
caso('el turno del pitch manda el precio con lo que incluye Y la demo, en dos mensajes, sin link (14-sep)',
    count($salidaPitch) === 2
    && strpos($salidaPitch[0], 'gokywebs.com/presupuestos/') === false
    && mb_stripos($salidaPitch[1], 'El primer paso es gratis') === 0);
...
// Estas comprobaciones necesitan las conversaciones definidas arriba.
...
$convVeg = ['tipo' => 'ecommerce', 'precio_dado' => true,
    'precio_cotizado' => '$30.000', 'mensualidad_cotizada' => '$30.000', 'precio_modelo' => 'mensual'];
...
/* 6. El link del presupuesto no se manda más (Pablo, 14-sep): la oración que
 * lo nombra se saca, y el link no se agrega. */
$cLink = ['tipo' => 'landing', 'transcript' => []];
$caroSinLink = wabot_link_presupuesto_completar('Son $20.000 por mes.' . "\n" . 'En el link del presupuesto tenés el detalle de todo lo que incluye.', $cLink, $cfg);
caso('la oración que nombra el link del presupuesto se saca, y el link no se agrega (14-sep)',
    $caroSinLink === 'Son $20.000 por mes.' && strpos($caroSinLink, (string)$cfg['tipos']['landing']['link']) === false);
...
caso('el mensaje del precio es la oferta, sin link ni párrafo de montos (14-sep)',
    (string)$cfg['tipos']['landing']['precio_ideal'] === 'Para {rubro} podemos hacer {propuesta}.' 
    && strpos((string)$cfg['tipos']['landing']['precio_ideal'], '{precio}') === false
    && stripos((string)$cfg['tipos']['landing']['precio_ideal'], 'pago único') === false);
caso('y el segundo mensaje es solo la oferta de la demo, sin montos (14-sep)',
    strpos((string)$cfg['msg_tres_pasos'], 'El primer paso es gratis') === 0
...
/* POSIBLE BUG (15-sep): textos.php ya no trae mensaje_laboral ni
 * mensaje_cliente_existente, pero redactor.php los sigue buscando y cae en
 * 'espera': al que manda un CV le contesta "el desarrollador ya tiene tu consulta". */
$cCv = conv_nueva(); wabot_conv_transcript($cCv, 'cliente', 'Hola, quiero mandarles mi CV'); $cCv['ultimo_cliente_ts'] = time();
clasifica(['otro']);
$rCv = wabot_responder('Hola, quiero mandarles mi CV', $cCv, $cfg);
caso('y los textos de esas dos salidas existen: el CV no se lleva la línea de espera del embudo',
    trim((string)($cfg['mensaje_laboral'] ?? '')) !== ''
    && trim((string)($cfg['mensaje_cliente_existente'] ?? '')) !== ''
    && ($rCv[0] ?? '') !== (string)$cfg['espera'], json_encode($rCv, JSON_UNESCAPED_UNICODE));
...
caso('el precio del proyecto combinado no repite el número del tipo base',
    $rComb && strpos($rComb[0], '180.000') === false && stripos($rComb[0], 'no sale de la lista') !== false, json_encode($rComb, JSON_UNESCAPED_UNICODE));
...
caso('con el form apagado trae el precio y los tres pasos, no el listado de datos (10-sep)',
    count($r2) === 2 && strpos($r2[0], '$20.000') !== false && strpos(implode("\n", $r2), '- Tu nombre') === false
    && mb_stripos($r2[1], 'El primer paso es gratis') !== false);
...
caso('con el form activo, lo mismo: el link sale recién con el sí del cliente',
    count($r2b) === 2 && mb_stripos($r2b[1], 'El primer paso es gratis') !== false && strpos(implode("\n", $r2b), 'gokywebs.com/form/') === false);
...
caso('la oferta actual explica la demo y los pasos, sin variantes retiradas',
    strpos($cfg['msg_tres_pasos'], 'El primer paso es gratis') !== false
    && strpos($cfg['msg_tres_pasos'], 'demo') !== false);


echo "— 1-sep: la apertura del anuncio da el rango real —\n";
$rangoTxt = wabot_texto_info('precio_sin_rubro', $cfg);
caso('"cuánto cuesta" antes del rubro no da montos: pregunta a qué se dedica (Pablo, 14-sep)',
    strpos($rangoTxt, '$') === false
    && strpos($rangoTxt, '{min}') === false && mb_stripos($rangoTxt, 'contame a qué te dedicás') !== false);
...
caso('el turno del precio son dos mensajes: el precio y, aparte, la demo (14-sep)',
    count($rSL) === 2 && strpos($rSL[0], '$30.000') !== false && mb_stripos($rSL[1], 'demo') !== false);
caso('con las dos formas, la demo aparte y sin el formulario (15-sep)',
    strpos(implode("\n", $rSL), 'gokywebs.com/form/') === false
    && mb_stripos($rSL[0], 'Y la podés contratar de dos formas') !== false
    && mb_stripos($rSL[1], 'El primer paso es gratis') !== false && mb_stripos(implode("\n", $rSL), 'seña') === false);
...
/* Desde el 10-sep el link NO sale con el precio: lo trae el sí del cliente, y
 * se manda UNA vez. */
$cSL2 = $cSL;
caso('tras el precio el link todavía no salió', wabot_link_form_ya_enviado($cSL2) === false);
clasifica(['otro']);
$rSL3 = wabot_engine('ok, dale', $cSL2, $cfg);
caso('el "ok, dale" a los tres pasos recibe el formulario',
    strpos(implode(' ', $rSL3), 'gokywebs.com/form/') !== false);
clasifica(['otro']);
$rSL4 = wabot_engine('ok, gracias', $cSL2, $cfg);
caso('y una vez mandado, el turno siguiente no lo repite: contesta la línea de espera',
    wabot_link_form_ya_enviado($cSL2) === true && strpos(implode(' ', $rSL4), 'gokywebs.com/form/') === false);
...
/* Solo se cotizan los cuatro tipos de la lista. */
...
caso('y el que de verdad sale nombra la seña del pago único y el mensual sin pago inicial (15-sep)',
    stripos(wabot_texto_pago_generico($cfg), 'seña para arrancar') !== false
    && stripos(wabot_texto_pago_generico($cfg), 'sin pago inicial') !== false);
...
caso('"es caro" no habla de cuotas (15-sep)', stripos((string)$cfg['caro'], 'cuotas') === false);
...
/* La pregunta directa por el monto de la seña tiene respuesta propia. */
