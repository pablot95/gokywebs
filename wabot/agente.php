...
function wabot_agente_intento($mensaje, &$conv, $cfg) {
    ...
    if (empty($conv['presentado_ts']) && wabot_texto_pregunta_cuanto_anticipo($mensaje)) {
        ...
        if (!empty($conv['precio_dado']) && $vAnt['precio'] !== '' && $vAnt['modelo'] === 'mensual') {
            return [wabot_precio_placeholders((string)($cfg['pago_cuanto_anticipo']
                ?? 'El primer pago es {precio} y es lo que se abona para arrancar con los cambios y dejar la web funcionando. A los 7 días comienza el plan mensual de {mensualidad}.'), $conv, $cfg)];
        }
    ...
    if (empty($conv['presentado_ts']) && wabot_texto_pregunta_cuando_se_paga($mensaje)) {
        ...
        return [wabot_texto_sin_modelo_viejo((string)($cfg['pago_antes_o_despues'] ?? 'La demo no se paga: primero te la mostramos y la ves, y recién si te gusta y querés avanzar se abona el primer pago para arrancar. A los 7 días de ese primer pago comienza el plan mensual.'))];
    }
 ...
 * primer pago de $60.000 se puede hacer en 12 cuotas; el plan es $30.000 por
