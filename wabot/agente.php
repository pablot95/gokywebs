...
function wabot_agente_intento($mensaje, &$conv, $cfg) {
    ...
    /* Desde el 10-sep el pago inicial es un número público: se dice. Con el
    ...
    if (empty($conv['presentado_ts']) && wabot_texto_pregunta_cuanto_anticipo($mensaje)) {
        ...
        if (!empty($conv['precio_dado']) && $vAnt['precio'] !== '' && $vAnt['modelo'] === 'doble') {
            return [wabot_precio_placeholders((string)($cfg['pago_cuanto_anticipo']
                ?? 'Depende de cómo la contrates: con el pago único arrancás con una seña de {sena} y el saldo lo abonás al entregar la web; con el servicio mensual no hay pago inicial, y la primera cuota, de {mensualidad}, arranca el armado.'), $conv, $cfg)];
        }
    ...
    if (empty($conv['presentado_ts']) && wabot_texto_pregunta_cuando_se_paga($mensaje)) {
        ...
        return [wabot_texto_sin_modelo_viejo((string)($cfg['pago_antes_o_despues'] ?? 'La demo no se paga: primero te la mostramos y, si te gusta, elegís cómo contratarla: con la seña del pago único o con la primera cuota del servicio mensual.'))];
    }
    ...
    if (in_array(($conv['fase'] ?? 'nuevo'), ['nuevo', 'menu', 'algo_diferente'], true)
        ...
        if ($falla) {
            ...
            if ($conv['ininteligibles'] === 1) {
                /* "Te lo pregunto de otra forma" sin haber preguntado nada:
                 * "asdfgh" como primer mensaje se lo llevaba (15-sep). */
                if (wabot_ultimo_texto_bot($conv) === '') {
                    $primero = trim((string)($cfg['ininteligible_primero'] ?? ''));
                    return [$primero !== '' ? $primero : 'Hola! No llegué a entender el mensaje. Contame a qué te dedicás o para qué sería la web y te ayudo.'];
                }
    ...
    if (empty($conv['precio_dado']) && empty($conv['tipo'])
        ...
        /* Con el rubro ya dicho no se le vuelve a preguntar a qué se dedica:
         * "doy clases de yoga y quiero vender mis cursos grabados" y después
         * "cuánto sale?" se llevaba "primero contame a qué te dedicás" (F04,
         * 15-sep). Cursos: los dos precios del desempate. Un tipo claro: su
         * precio. Lo demás (sistema, híbrido) lo resuelve el modelo. */
        // Solo lo que dijo ANTES: "cuánto sale una web para mostrar mis
        // servicios y cuánto una tienda?" no es un rubro dicho (Q07, 15-sep).
        $convPrevio = $conv;
        $trPrevio = (array)($convPrevio['transcript'] ?? []);
        $ultPrevio = end($trPrevio);
        if ($ultPrevio && ($ultPrevio['q'] ?? '') === 'cliente') {
            array_pop($trPrevio);
    ...
    if (wabot_info_por_palabras($mensaje, $faseAtajo) === 'carga'
        /* Pero "tengo un taller, quiero vender insumos y más adelante subir
         * cursos" no pregunta quién carga: cuenta para qué es la web, y se
         * llevaba este texto en vez del precio (batería del 14-sep). */
        && !wabot_carga_es_el_proyecto($mensaje, $conv)) {
        return [wabot_texto_info('carga', $cfg)];
    }

    /* "No me interesa el mantenimiento, no creo que sea necesario mensualmente"
     * se llevaba el detalle del plan con los $10.000 de más cambios, y "lo
     * quiero en un solo pago y mantenerla yo", lo mismo (Pablo, 14-sep). El
     * texto oficial, sin el modelo. */
    $clavePlan = $faseAtajo !== 'derivado' ? wabot_info_por_palabras($mensaje, $faseAtajo) : null;
    if (in_array($clavePlan, ['plan_es_servicio', 'un_solo_pago'], true)) {
     ...
     * ya arrancaba diciendo que lo era (3-sep). Determinista y antes del
     ...
     * el texto de 'caro' —entrar con el pago inicial y olvidarse de todo lo
    ...
    for ($vuelta = 0; $vuelta < WABOT_AGENTE_MAX_VUELTAS; $vuelta++) {
        ...
        if (!$llamadas) {
            ...
            if (wabot_texto_dice_monto_de_cuota($texto, $cfg)) {
                wabot_log('error', ['donde' => 'agente', 'msg' => 'monto de cuota inventado', 'texto' => mb_substr($texto, 0, 140)]);
                return null;
            }

            if (wabot_texto_mezcla_formas($texto, $cfg)) {
                wabot_log('error', ['donde' => 'agente', 'msg' => 'mezcla las dos formas', 'texto' => mb_substr($texto, 0, 140)]);
                return null;
            }
            ...
            /* El comodín del desarrollador contestando algo que no preguntó
             * nada. Ver wabot_texto_comodin_desarrollador(): la regla estaba
             * en el prompt con los ejemplos exactos y salió igual. */
            if (wabot_texto_comodin_desarrollador($texto) && !wabot_mensaje_pregunta_algo($mensaje)) {
                wabot_log('error', ['donde' => 'agente', 'msg' => 'comodin sin pregunta', 'texto' => mb_substr($texto, 0, 140)]);
                return null;
            }

            /* Una fecha de entrega distinta de la que dice {entrega}. */
            if (wabot_texto_promete_otra_entrega($texto)) {
                wabot_log('error', ['donde' => 'agente', 'msg' => 'otra fecha de entrega', 'texto' => mb_substr($texto, 0, 140)]);
                return null;
            }

            /* "Quedamos a la espera de la demo": la demo la hacemos nosotros. */
            if (wabot_texto_espera_la_demo($texto)) {
                wabot_log('error', ['donde' => 'agente', 'msg' => 'dice esperar la demo', 'texto' => mb_substr($texto, 0, 140)]);
                return null;
            }
        ...
        if ($exacta !== null) {
            ...
            /* Una línea de reconocimiento delante del texto exacto.
             *
             * Fátima enumeró siete secciones para su web de campeonatos
             * (tablas, goleadores, sanciones, equipos, jugadores) y Henry
             * explicó en tres renglones su servicio de enfermería domiciliaria:
             * los dos recibieron el mismo pitch de siempre, sin una palabra de
             * lo que habían escrito (Pablo, 3-sep). No es que el modelo no lo
             * entienda —lo entiende— es que su texto se descartaba entero al
            ...
            // Con UNA alcanza: el texto del modelo se descartó, así que la
            // pregunta del cliente no la contestó nadie (caso Rodo, 3-sep).
            $sinContestarExacta = wabot_agente_empujon_preguntas($mensaje, $salidaExacta, $conv, $cfg, 1);
 ...
 * Eso nunca fue lo que cobramos: la seña del pago único y el servicio mensual
 * son montos fijos por tipo de web, cargados en la config (15-sep).
 ...
 * pago inicial de $60.000 se puede hacer en 12 cuotas; el plan es $30.000 por
 ...
 * al texto de reglas. El total financiado ("$60.000 en 12 cuotas") y la
 * mensualidad ("$30.000 por mes") no son el valor de una cuota.
 */
function wabot_texto_dice_monto_de_cuota($texto, $cfg = null) {
    $t = (string)$texto;
    if ($t === '') return false;
    /* La cuota del servicio mensual ES la mensualidad, un monto público:
     * "pagás únicamente la primera cuota de $20.000" se descartaba y el
     * respaldo repetía el texto de pago entero (F01, 15-sep). Pasa solo ese
     * monto, en singular y con el servicio mensual nombrado. */
    if (is_array($cfg) && preg_match('/\b(mensual\w*|por mes|suscripci\w*)\b/iu', $t)) {
        $mensualidades = [];
        foreach ((array)($cfg['tipos'] ?? []) as $d) {
            $n = (int)wabot_monto_a_numero((string)($d['mensualidad'] ?? ''));
            if ($n > 0) $mensualidades[$n] = true;
        }
        $t = preg_replace_callback('/\b(primera|la|tu|su|esa)\s+cuota(\s+mensual)?\s*,?\s*(de|es de|queda en|sale)?\s*,?\s*\$\s?([\d.]+)/iu',
            function ($m) use ($mensualidades) {
                return isset($mensualidades[(int)str_replace('.', '', $m[4])]) ? 'la mensualidad' : $m[0];
            }, $t);
    }
    return (bool)(
...
/**
 * Las dos formas mezcladas (auditoría del 15-sep). Ningún guard lo miraba:
 * pasaban "con el servicio mensual arrancás con una seña de $40.000", "el pago
 * único de $20.000", "el mantenimiento es obligatorio", "tiene una permanencia
 * mínima de 12 meses" y "el saldo en 8 pagos de $30.000". Se mira oración por
 * oración; la que nombra las dos formas se deja pasar.
 */
function wabot_texto_mezcla_formas($texto, $cfg) {
    $crudo = (string)$texto;
    if (trim($crudo) === '') return false;
    if (preg_match('/\b\d{1,2}\s+(pagos|cuotas)\s+(de|por)\s+\$/iu', $crudo)) return true;

    // Montos con formato de miles; "$10.000 más por mes" es un adicional, no la mensualidad.
    $montos = function ($s) {
        $out = [];
        if (preg_match_all('/\$\s?(\d{1,3}(?:\.\d{3})+)(?!\d)(?!\s*(pesos\s*)?m[aá]s\b)/u', $s, $m)) {
            foreach ($m[1] as $x) $out[] = (int)str_replace('.', '', $x);
        }
        return $out;
    };
    $mensualidades = [];
    ...
    foreach ((array)($cfg['tipos'] ?? []) as $d) {
        $n = (int)wabot_monto_a_numero((string)($d['mensualidad'] ?? ''));
        if ($n > 0) $mensualidades[$n] = true;
        foreach (['precio', 'sena'] as $k) {
            $p = (int)wabot_monto_a_numero((string)($d[$k] ?? ''));
            if ($p > 0) $delUnico[$p] = true;
        }
    }

    foreach (preg_split('/(?<=[.!?\n])\s+/u', $crudo) as $oracion) {
        $n = wabot_normalizar_frase($oracion);
        if ($n === '') continue;
        $mensual = (bool)preg_match('/\b(servicio mensual|el mensual|suscripcion|mensualidad|abono mensual|plan mensual)\b/u', $n);
        $unico   = (bool)preg_match('/\b(pago unico|un solo pago|unico pago|una sola vez)\b/u', $n);

        if (preg_match('/\bobligatori\w*/u', $n) && preg_match('/\b(mensual\w*|plan|mantenimiento|abono|suscripcion)\b/u', $n)
            && !preg_match('/\b(no|ni|nada|tampoco)\b.{0,12}\bobligatori/u', $n)) return true;
        if (preg_match('/\bpermanencia\b/u', $n)
            && !preg_match('/\b(sin|no hay|no tiene|no tenes|no tenemos|no lleva|ninguna|cero)\b.{0,15}\bpermanencia\b/u', $n)) return true;

        if ($mensual && !$unico) {
            if (preg_match('/\b(sena|senia|saldo)\b/u', $n) && !preg_match('/\b(sin|no hay|no tiene|no lleva|ni)\b.{0,10}\b(sena|senia|saldo)\b/u', $n)) return true;
            if (preg_match('/\bpago inicial\b/u', $n) && !preg_match('/\b(sin|no hay|no tiene|no lleva|ningun|ni)\b.{0,12}\bpago inicial\b/u', $n)) return true;
            if (preg_match('/\b(la web es tuya|es tuya desde|queda a tu nombre)\b/u', $n) && !preg_match('/\b12 meses\b/u', $n)) return true;
            foreach ($montos($oracion) as $monto) {
                if (!isset($mensualidades[$monto])) return true;
            }
        }
        if ($unico && !$mensual) {
            foreach ($montos($oracion) as $monto) {
                if (isset($mensualidades[$monto]) && !isset($delUnico[$monto])) return true;
            }
        }
    }
    return false;
...
function wabot_texto_promete_info_sin_entregar($texto) {
    ...
    /* "Si me contás a qué te dedicás, te paso el presupuesto exacto" no promete
     * algo que falte mandar: pide el dato para poder darlo. Se descartaba y el
     * respaldo terminó derivando la charla (F06, 15-sep). */
    $pide = '(si me (contas|decis|pasas|comentas|indicas|contestas)|contame|decime|pasame|comentame|(cuando|apenas|en cuanto) me (digas|cuentes|pases|contes))';
    if (preg_match('/\b' . $pide . '\b.{0,70}\b(te paso|te muestro|te comparto|te envio|te dejo)\b/u', $t)
        || preg_match('/\b(te paso|te muestro|te comparto|te envio|te dejo)\b.{0,60}\b' . $pide . '\b/u', $t)) {
        return false;
    }
    return (bool)preg_match('/\b(te paso|te muestro|te comparto|te env[ií]o|te dejo|aca va|aca tenes|aca esta)\b.{0,40}\b(precio|detalle|informacion|el link|todo lo que incluye|presupuesto|el valor)\b/u', $t);
}
...
/**
 * "Ese pago es si o si. Es decir fijo o es opcional?"
 *
 * Ana Sloog, 3-sep: acababa de recibir el bloque entero del mantenimiento —que
 * arranca diciendo "El mantenimiento es opcional"— y preguntó eso. El bot le
 * volvió a pegar el bloque entero, palabra por palabra. La respuesta eran dos
 * palabras y salieron cuatro renglones repetidos.
 *
 * (El anti-repetición no lo agarró porque la primera vez el texto traía además
 * el párrafo de redes, así que las dos tandas no eran idénticas.)
 *
 * Regla de Pablo: a una pregunta cerrada se le contesta primero la respuesta
 * concreta, y recién después se amplía si hace falta.
 */
...
/**
 * "Tengo un taller, quiero vender insumos y más adelante subir cursos",
 * "quiero una web para subir mis productos": el verbo de carga describe el
 * PROYECTO, no pregunta quién carga. Solo antes de cotizar, sin signo de
 * pregunta y sin "puedo", "quién" o "los cargo yo" (batería del 14-sep: el
 * taller se llevaba el texto de carga en vez de la cotización).
 */
function wabot_carga_es_el_proyecto($mensaje, $conv) {
    if (!empty($conv['precio_dado']) || !empty($conv['tipo'])) return false;
    if (strpos((string)$mensaje, '?') !== false) return false;
    $t = wabot_normalizar_frase((string)$mensaje);
    if (preg_match('/\b(puedo|podemos|podre|podria|se puede|se pueden|quien|cargan ustedes|lo cargo|los cargo|las cargo|lo subo|los subo|las subo)\b/u', $t)) return false;
    return (bool)preg_match('/\b(quiero|queremos|quisiera|necesito|necesitamos|busco|buscamos|me gustaria|nos gustaria|la idea es|para|mas adelante|a futuro)\b.{0,40}\b(agregar|anadir|sumar|cargar|subir|publicar|vender)\b/u', $t);
}

/**
 * La respuesta corta, sacada de lo que el bot acaba de decir.
 *
 * No consulta la config ni el modelo: mira la última tanda propia y le
 * responde la polaridad que esa misma tanda ya tenía. Así no puede
 * contradecirse, que es el único riesgo real de contestar corto. Si lo último
...
function wabot_respuesta_obligatorio($conv, $cfg, $mensaje = '') {
    /* La TANDA entera, no el último globo: desde el 14-sep el precio sale en
     * dos mensajes y el segundo es la oferta de la demo, así que "¿es
     * obligatorio pagar todos los meses?" miraba solo la demo y el plan
     * quedaba en manos del modelo ("Sí, es obligatorio porque…"). */
    $tanda = [];
     ...
     * último que dijo el bot habla del plan, se contesta respuesta_plan_obligatorio:
     * desde el 15-sep, que no, que es una de las dos formas de contratar. */
    if (preg_match('/\b(plan mensual|servicio mensual|abono mensual|mensualidad|mantenimiento|por mes|suscripcion)\b/u', $u)) {
        ...
        return trim((string)($cfg['respuesta_plan_obligatorio']
            ?? 'No: el servicio mensual es una de las dos formas de contratar la web. Si preferís, la pagás una sola vez y no tenés abono mensual.'));
    }
 ...
 *
 * La clienta de Estética Integral mandó el link de su catálogo de WhatsApp
 * —material suyo, no una duda— y se llevó "esa duda te la va a poder contestar
 * el desarrollador cuando te escriba" (3-sep). Es, de todos los errores, el
 * que más deja en evidencia que del otro lado hay un bot que no entendió.
 *
 * La regla ya estaba escrita en el prompt, con estos mismos ejemplos y todo
 * (wabot_agente_sistema: "Este es mi face", "Que lo haga vía wasap"), y no
 * alcanzó — como siempre que algo importante vive solo en el prompt. Acá el
 * turno se descarta y contesta el motor de reglas, que para un mensaje sin
 * pregunta dice algo que corresponde.
 */
function wabot_texto_comodin_desarrollador($texto) {
    $t = wabot_normalizar_frase((string)$texto);
    if ($t === '') return false;
    return (bool)preg_match(
        '/\b(esa|esta|tu|eso|esto)\b.{0,30}\b(duda|consulta|pregunta)?\b.{0,30}\b(desarrollador|pablo)\b/u', $t
    ) && (bool)preg_match('/\b(contestar|responder|resolver|contesta|responde|ver|decir)\b/u', $t)
    // Y en el otro orden: "eso lo va a poder responder Pablo cuando te contacte".
    || (bool)preg_match(
        '/\b(contestar|responder|resolver)\b.{0,20}\b(el desarrollador|pablo)\b/u', $t
    );
}

/**
 * Una sola fecha de entrega para toda la conversación.
 *
 * Aura32, 3-sep: 11:21 "Hoy a la tarde estará lista la demo" y 11:22 "la demo
 * te llega mañana". El segundo mensaje es el texto de la config con {entrega},
 * que lo resuelve wabot_dia_entrega() —la única fuente de verdad, con su corte
 * a las 11— y el primero lo escribió el modelo de memoria.
 *
 * Solo corta cuando CONTRADICE: si el modelo dice el mismo día que
 * wabot_dia_entrega(), el mensaje sale igual. Prometer una fecha equivocada es
 * un problema; nombrar la correcta con sus palabras, no.
 */
function wabot_texto_promete_otra_entrega($texto, $ts = null) {
    $t = wabot_normalizar_frase((string)$texto);
    if ($t === '') return false;
    /* Tiene que hablar de la entrega de la demo, no de cualquier "hoy". El
     * segundo patrón es para cuando la nombra con un pronombre ("te la dejo
     * lista mañana"), que en esta parte del embudo es siempre la demo. */
    if (!preg_match('/\b(demo|muestra|predise\w*|boceto|propuesta)\b/u', $t)
        && !preg_match('/\bte la (dejo|dejamos|mando|mandamos|paso|pasamos|envio|enviamos|tengo|tenemos)\b/u', $t)) return false;
    $entrega = function_exists('wabot_dia_entrega') ? wabot_dia_entrega($ts) : ['palabra' => ''];
    $palabra = (string)($entrega['palabra'] ?? '');

    $diceHoy = (bool)preg_match('/\b(hoy|esta tarde|en el dia|mas tarde|dentro de un rato|en un rato)\b/u', $t);
    $diceManana = (bool)preg_match('/\b(manana|el lunes|el martes|el miercoles|el jueves|el viernes)\b/u', $t);

    if ($palabra === 'hoy' && $diceManana && !$diceHoy) return true;
    if ($palabra === 'mañana' && $diceHoy && !$diceManana) return true;
    return false;
}

/**
 * "Quedamos a la espera de la demo."
 *
 * Se lo dijo a Natalia después de que ella completara todo (3-sep). La demo la
 * prepara Gokywebs: el que espera es el cliente, no nosotros. Dicho al revés
 * suena a que del otro lado nadie va a hacer nada, que es exactamente lo
 * contrario de lo que hay que transmitir en ese momento de la venta.
 */
function wabot_texto_espera_la_demo($texto) {
    $t = wabot_normalizar_frase((string)$texto);
    if ($t === '') return false;
    return (bool)preg_match(
        '/\b(quedamos|quedo|estamos|seguimos)\b.{0,18}\b(a la espera|esperando|al aguardo)\b.{0,25}\b(demo|muestra|predise\w*|boceto)\b/u',
        $t
    );
}
...
/**
 * Lo mismo que wabot_agente_prefijo_humano(), pero para el texto EXACTO del
 * precio, que es donde se nota más que el bot no acusa recibo de nada.
 *
 * Se separan porque los frenos no pueden ser los mismos: el prefijo de la
 * derivación prohíbe hablar del servicio ("podemos", "incluye", "tiene"), y
 * acá el reconocimiento es JUSTAMENTE sobre lo que el cliente necesita
 * ("entonces necesitás una web donde puedas cargar las tablas y los
 * goleadores"). Lo que no cambia es lo que no puede salir mal: ni un monto, ni
 * un link, ni una pregunta, ni una promesa, ni una condición comercial.
 *
 * Si el modelo no escribió nada —el caso más común, porque suele mandar la
 * llamada a la herramienta y nada más— devuelve el texto exacto tal cual y no
 * cambia nada.
 */
function wabot_agente_prefijo_acuse($textoModelo, $exacta, $conv, $cfg) {
    ...
    if (preg_match('/[?¿$]|\d{3,}|https?:|www\.|\.com\b|\{/u', $p)) return $exacta;
    $n = wabot_normalizar_frase($p);
    if ($n === '') return $exacta;

    // Nada de plata, plazos, condiciones ni promesas: eso lo dice el texto oficial.
    if (preg_match('/\b(precio|precios|valor|cuesta|sale|sena|cuotas?|pago|abon\w*|descuento|gratis'
        . '|plazo|dias|semanas|manana|hoy|demo|predise\w*|muestra|formulario|link|presupuesto)\b/u', $n)) return $exacta;
    // Ni derivaciones, ni anotaciones, ni pedidos de datos.
    if (preg_match('/\b(te paso|te comunico|te derivo|te contact\w*|te escrib\w*|te llam\w*|desarrollador|pablo|equipo'
        . '|en breve|a la brevedad|enseguida|te aviso|quedo|quedamos|contame|decime|pasame|mandame'
        . '|anot\w*|registr\w*|sum[eé]|tom[eé] nota)\b/u', $n)) return $exacta;
    if (wabot_texto_promete_cierre($p) || wabot_texto_promete_info_sin_entregar($p)) return $exacta;
    if (wabot_texto_comodin_desarrollador($p)) return $exacta;
    if (function_exists('wabot_texto_parece_interno') && wabot_texto_parece_interno($p)) return $exacta;
    if (function_exists('wabot_frase_retirada') && wabot_frase_retirada($p)) return $exacta;

    if (function_exists('wabot_castellanizar')) $p = wabot_castellanizar($p);
    $p = trim(preg_replace('/[\x{1F000}-\x{1FAFF}\x{2600}-\x{27BF}\x{FE0F}]/u', '', $p));
    $p = trim(str_replace(['¿', '¡'], '', $p));

    /* El texto del precio ya arranca con "Perfecto,". Si el acuse también, se
     * le saca la muletilla al acuse: dos "Perfecto" seguidos suenan peor que
     * ninguno. Si de eso no queda nada, no había acuse, había relleno. */
    if (preg_match('/^\s*(perfecto|dale|listo|buenisimo|buenísimo|barbaro|bárbaro|genial|ok|entiendo|entendido)\b[\s,.:!–—-]*/iu', $p)) {
        $sinMuletilla = trim(preg_replace('/^\s*(perfecto|dale|listo|buenisimo|buenísimo|barbaro|bárbaro|genial|ok|entiendo|entendido)\b[\s,.:!–—-]*/iu', '', $p));
        $arrancaIgual = (bool)preg_match('/^\s*(perfecto|dale|listo|buenisimo|buenísimo|genial|ok)\b/iu', (string)$exacta);
        if ($sinMuletilla === '') return $exacta;
        if ($arrancaIgual) {
            $p = mb_strtoupper(mb_substr($sinMuletilla, 0, 1)) . mb_substr($sinMuletilla, 1);
        }
    }
    // Un acuse tiene que decir algo: menos de cuatro palabras es una muletilla.
    if (count(array_filter(preg_split('/\s+/u', wabot_normalizar_frase($p)))) < 4) return $exacta;
    if (!preg_match('/[.!]$/u', $p)) $p .= '.';
    return $p . "\n" . $exacta;
}
 ...
 *
 * Con el texto libre del modelo pide DOS temas o más: una pregunta sola la
 * contesta bien el flujo normal y perseguirla ahí solo duplicaría.
 *
 * Pero cuando la tanda salió por un texto EXACTO o TERMINAL, el texto del
 * modelo se descartó entero (agente.php: `return [$terminal]` / `$exacta`), así
 * que no hubo "flujo normal" que contestara nada: ahí alcanza UNA. Ese es el
 * caso de Rodo (3-sep), que preguntó "tiene algún pago mensual o la página me
 * queda de por vida una vez pagada?" junto con su rubro: el pitch salió exacto,
 * su única pregunta la detectó wabot_preguntas_del_mensaje() —clave
 * 'mantenimiento'— y este empujón la descartó por venir sola. El cliente
 * recibió el precio y la oferta de la demo, y nadie le contestó lo que había
 * preguntado. Antes de avanzar de etapa hay que mirar si quedó una pregunta sin
 * contestar, que es justo lo que pidió Pablo.
 *
 * El logo y el idioma tienen su propio empujón desde antes: quedan afuera para
 * no contestarlos dos veces.
 */
function wabot_agente_empujon_preguntas($mensaje, $salida, &$conv, $cfg, $minimo = 2) {
    $claves = wabot_preguntas_del_mensaje($mensaje, $conv, $conv['fase'] ?? null);
    if (count($claves) < max(1, (int)$minimo)) return null;
...
function wabot_agente_tools($cerrada = false, $postdemo = false) {
    ...
    return [
        [
            'name' => 'dar_precio',
            'description' => 'Devuelve el precio de un tipo de web: lo que incluye y las DOS formas de contratarla (pago único con seña, o servicio mensual). Usala SOLO cuando ya sabés con certeza qué tipo necesita el cliente. El texto que devuelve hay que incluirlo tal cual en la respuesta.',
            'parameters' => [
                'type' => 'object',
                'properties' => [
                    'tipo' => [
...
function wabot_agente_ejecutar($nombre, $args, &$conv, $cfg, $mensaje = '') {
    switch ($nombre) {
        ...
        case 'dar_precio':
            ...
            /* Si el modelo no lo mandó (o mandó algo que no pasó la
             * validación), se saca de lo que escribió el cliente antes de
            ...
            if ($eraPitch) {
                return [
                    ...
                    'nota'  => 'Mandá este texto tal cual, entero y sin nada adelante ni atrás: es lo que le podemos hacer, lo que incluye y las dos formas de contratarla. La oferta de la demo gratis sale sola en un segundo mensaje: no la escribas vos. El link del formulario NO lo mandes ahora: se lo pasás recién cuando conteste que quiere la demo, con consultar_info(\'prediseno\').',
            ...
            return [
                'texto' => $precio[0],
                'nota'  => 'Mandá este texto tal cual, solo y sin preámbulo y respetando los saltos de línea. La oferta de la demo sale sola en un segundo mensaje: NO le agregues introducciones ni frases de beneficio, y no la escribas vos.',
                'aparte' => array_slice($precio, 1),
            ];
        ...
        case 'consultar_info':
            ...
            /* Y no se le describe un producto que no es el suyo.
             *
             * La clienta de Estética Integral pidió una landing page tres
             * veces, se le cotizó el sitio profesional, se le creó el boceto y
             * se le prometió la demo. Después dijo "Tengo un catálogo de
             * productos" —un dato de su negocio— y se llevó
             * info.como_funciona_tienda entera: "el cliente entra, ve el
             * catálogo, arma el carrito y paga desde la web" (3-sep). Un dato
             * nuevo no puede hacer que el bot descarte toda la charla anterior
             * y le explique otra web.
             *
             * Si de verdad quiere pasarse a vender online, eso tiene su propio
            ...
            $clavesDeTipo = [
                ...
                'comisiones'              => ['ecommerce', 'catalogo'],
                'inscripcion'             => ['elearning', 'lms'],
                'fotos_propiedad'         => ['inmobiliaria'],
            ];
            $tipoActual = (string)($conv['tipo'] ?? '');
            if ($tipoActual !== '' && !empty($conv['precio_dado']) && isset($clavesDeTipo[$clave])
                && !in_array($tipoActual, $clavesDeTipo[$clave], true)) {
                $label = wabot_tipo_label($tipoActual, $cfg);
                return [
                    'error' => 'Esa clave describe otro tipo de web. A este cliente ya se le cotizó ' . ($label !== '' ? $label : $tipoActual)
                        . ', así que contestarle cómo funciona la tienda es explicarle un producto que no es el suyo.',
                    'nota'  => 'No uses esa clave. Si lo que dijo es un dato de su negocio, tomalo como eso y confirmáselo en una línea sin cambiarle el proyecto. Si lo que quiere es AGREGAR la venta online, decíselo con esas palabras y preguntale si quiere que lo veamos, sin describir la tienda todavía.',
                ];
            }
            ...
            if ($clave === 'prediseno') {
                ...
                if (empty($conv['link_form_enviado']) && !empty($conv['precio_dado']) && !empty($conv['cta_muestra'])
                    ...
                    return ['error' => 'El cliente todavía no dijo que quiere la demo: lo que escribió no es un sí.',
                            'nota'  => 'No le mandes el formulario. Si te contó algo más de su negocio, reconocelo en UNA línea nombrando lo que dijo y preguntale si quiere que le preparemos la demo. Si te hizo una pregunta, contestala. No repitas el precio ni la oferta de la demo. El link sale recién cuando conteste que sí.'];
                }
                $conv['fase'] = 'prediseno';
            ...
            if ($clave === 'mantenimiento') {
                return wabot_agente_agregar_cta([
                    ...
                    'nota' => 'Contestá con esta información y nada más: los montos van idénticos. Si preguntó si es obligatorio, la respuesta es que no: el servicio mensual es una de las dos formas, y eso va primero.',
                ], $conv, $cfg);
            }
            if ($clave === 'pago') {
                return wabot_agente_agregar_cta([
                    'texto' => wabot_texto_pago($conv, $cfg),
                    'nota' => 'Contestá con esto tal cual: son las dos formas de pago de lo cotizado, con sus montos; no los recalcules ni los redondees.',
                ], $conv, $cfg);
            }
            if ($clave === 'hosting') {
                return wabot_agente_agregar_cta([
                    'texto' => wabot_texto_hosting($conv, $cfg, $mensaje),
                    'nota' => 'Contestá con esto tal cual. El monto de la renovación del hosting va solo si está en este texto: no lo agregues de memoria.',
                ], $conv, $cfg);
            }
            ...
            if ($clave === 'rangos') {
                return ['texto' => wabot_texto_rangos($cfg),
                        'nota' => 'Contestá con esto tal cual: sin saber a qué se dedica o para qué es la web no se dan precios, y el texto se lo pregunta. Con esa respuesta, cotizá con dar_precio.'];
            }
            ...
            if ($clave === 'objecion_precio') {
                if (empty($conv['tipo']) || empty($conv['precio_dado'])) {
                    return ['error' => 'Todavía no le diste un precio a este cliente: esta respuesta habla de un precio que nunca vio.',
                ...
                /* "Hay descuento?" se contesta con el no y las dos formas
                 * (15-sep), igual que en wabot_regateo_responder(). */
                if (preg_match('/\b(descuento|descuentos|rebaja|rebajita|bonificacion)\b/u', wabot_normalizar_frase($mensaje))
                    && !wabot_regateo_es_contraoferta($mensaje)
                    ...
                    return ['texto' => wabot_texto_descuento($conv, $cfg), 'exacta' => true,
                            'nota' => 'Mandá esto tal cual: no hay descuentos, y ahí están las dos formas con sus montos. No agregues ningún número.'];
                }
                return wabot_agente_agregar_cta([
        ...
        case 'cuotas_sin_interes':
            return ['error' => 'Esa herramienta ya no existe: el bot no cobra ni manda datos de pago.',
                    'nota'  => 'Derivá con causa pago_explicito y dejá que el sistema mande el aviso: no anuncies vos el contacto. Nunca escribas el CBU, el alias, el titular ni un link de pago o de suscripción. Los montos del pago único, la seña y el servicio mensual sí se pueden repetir, con consultar_info(\'precio_cotizado\').'];
...
- Si pide los precios sin decir para qué es la web, NO des precios (Pablo, 14-sep): primero averiguá a qué se dedica o qué necesita —consultar_info('rangos') se lo pregunta— y recién con eso cotizá con dar_precio.
...
- DESDE EL 13-SEP TODAS LAS WEBS TRAEN UN PANEL DE ADMINISTRACIÓN PARA EDITAR LOS TEXTOS Y LAS IMÁGENES, sin costo extra: sitio profesional, ecommerce, inmobiliaria y cursos. Si el cliente pregunta si puede editar o actualizar la web él mismo, o si es autoadministrable, la respuesta es SÍ, va por consultar_info('carga') y se cotiza como siempre. Nunca digas que un tipo de web no trae panel ni que los textos o las fotos los cambiamos solo nosotros.
- Lo de esta sección es OTRA cosa y es SOLO si el cliente lo pide él mismo, explícitamente: que su web viva de publicar contenido nuevo todo el tiempo —cargar noticias, notas o artículos, sumar secciones nuevas sin depender de nadie —por ejemplo un diario o portal de un club, una revista, un sitio con novedades que cambian todo el tiempo— eso NO es un sitio profesional común, aunque el rubro por sí solo (un club, una institución) sugiera lo contrario. Es un desarrollo a medida.
...
(Pablo, 3-sep, después de leer las charlas del día. Casi todo lo que se sintió
"automático" ese día falla en alguno de estos cinco.)
1. ¿Hay una pregunta directa en el último mensaje? Contestala PRIMERO, antes de seguir con el pitch, el precio o la demo. Aunque sea una sola y venga mezclada con otra cosa: el que pregunta "tiene algún pago mensual?" junto con su rubro tiene que recibir esa respuesta, no solo el precio. Y si la pregunta es CERRADA (sí/no, "es fijo o es opcional?"), contestá la respuesta concreta en una línea y recién después ampliá si hace falta: volver a pegar el bloque entero que ya mandaste es lo que te hace parecer menos inteligente.
2. ¿Esto ya me lo dijo? Mirá la ficha y la charla antes de preguntar. Si ya contestó —incluso con un NO ("no tengo ninguna referencia", "no tengo logo")— eso es una respuesta y no se vuelve a preguntar. Repreguntar algo que el cliente acaba de contestar es lo que más delata a un bot.
3. ¿Ya completó el formulario? Si en la ficha figura que sí, NUNCA se lo vuelvas a ofrecer ni le pidas de nuevo los datos que ya mandó. Puede haberlo completado desde Instagram y estar escribiéndote por WhatsApp: es la misma persona.
4. ¿Lo que acaba de decir contradice o modifica algo anterior? Un dato nuevo no borra la charla. Si viene pidiendo un sitio profesional y menciona que tiene un catálogo de productos, eso NO lo convierte en un ecommerce: tomá el dato, confirmáselo, y si te parece que quiere vender online preguntáselo, no se lo supongas.
5. ¿Mi respuesta demuestra que entendí lo que escribió? Si enumeró funciones o te explicó su servicio en detalle, nombrá brevemente eso —con sus palabras— antes de seguir. Una respuesta genérica a un mensaje detallado se lee como que no lo leyó nadie.

REGLAS QUE NO PODÉS ROMPER
- Los precios los conocés SOLO llamando a dar_precio. Nunca los digas de memoria ni los inventes. El texto que te devuelve ya trae lo que incluye y las dos formas de contratarla: mandalo completo, no lo recortes. Nunca mandes links de gokywebs.com/presupuestos: desde el 14-sep el detalle va en el mismo mensaje. Si más adelante te pide que se lo repitas, usá consultar_info('precio_cotizado').
...
- Si te pregunta el precio ANTES de decirte qué tipo de web necesita ("cuánto sale?", "qué precio tiene?"), NO te escapes con una respuesta de relleno: usá consultar_info('precio_sin_rubro'), que le pregunta a qué se dedica o para qué es la web. Sin saber para qué es NO se dan precios (Pablo, 14-sep): primero averiguás y después cotizás con dar_precio.
- Si pregunta CÓMO TRABAJAMOS o cómo es el paso a paso ("cómo se manejan", "cómo arrancamos", "cómo sigue"), usá consultar_info('proceso'). Ese texto explica la demo gratis y las dos formas de contratar la web. Si quiere los montos, es otra pregunta.
...
- "Cuánto sale", "cuánto cuesta", "el más barato" o "la más completa" piden un PRECIO. Con el tipo confirmado, dar_precio; sin tipo confirmado, consultar_info('rangos'). NUNCA contestes eso con las formas de pago, y nunca cotices el tipo más caro solo porque pidió "la más completa": el tipo sale de lo que vende o hace, no del adjetivo. El precio son las DOS formas de contratar la misma web, con sus montos: pago único y servicio mensual.
- Si el precio ya se dio y lo vuelve a preguntar ("cuál era el precio?", "cuánto quedaba?"), repetilo con dar_precio del mismo tipo o consultar_info('precio_cotizado'): la respuesta corta con las dos formas.
- "No me interesa el mantenimiento", "no creo que haga falta todos los meses" o "lo quiero de vez en cuando" NO preguntan por el plan: no quieren pagar por mes. Van por consultar_info('plan_es_servicio'), que les ofrece el pago único, sin abono mensual; nunca les mandes el detalle del plan con los montos de más cambios. "Puedo pagarla una sola vez?" o "tienen pago único?" va por consultar_info('un_solo_pago'), que trae ese valor.
- "Hay algo mensual?", "el mantenimiento es obligatorio?" o "cuánto es por mes?" van por consultar_info('mantenimiento'): el servicio mensual es UNA de las dos formas de contratar la web, no es obligatorio (con el pago único no hay abono), y esa respuesta ya trae los montos. Las comparaciones con Tiendanube/Wix/Shopify van a manejar_objecion('plataforma'). "Por qué no uso Tiendanube que es gratis" es esto, NUNCA consultar_info('tecnologia'): esa clave es solo si preguntan de qué lenguaje o hosting está hecha la web, no para comparar con una plataforma competidora. manejar_objecion('plataforma') te devuelve el argumento correcto: allá la página la arma el cliente con una plantilla y paga por mes igual; acá la hacemos nosotros, a medida, y nos ocupamos de todo. Nunca digas "sin costos mensuales" a secas: con el servicio mensual los hay; con el pago único no hay abono, pero el hosting y el dominio se renuevan una vez al año después del primer año.
...
- El CBU, el alias, el titular y los links de pago o de suscripción NO los mandás nunca: eso lo arregla el desarrollador. Si te preguntan cómo se paga, usá consultar_info('pago'): trae las dos formas con sus montos.
- CUÁNDO SE PAGA es una pregunta de pago y va por consultar_info('pago') SIEMPRE, aunque el cliente lo escriba corto o con errores ("y después se abona?", "se paga antes o después?", "cómo es el pago?"). Nunca la contestes de memoria ni con lo que hayas leído más arriba en la charla: la respuesta sale de la herramienta y se manda con los montos que trae. Con el pago único se arranca con una seña y el saldo va al entregar la web; con el servicio mensual no hay pago inicial y la primera cuota arranca el armado. Las dos, después de ver la demo.
- La seña del pago único es un MONTO FIJO según el tipo de web (el que te devuelve la herramienta), nunca un porcentaje ni una mitad: si escribís "50%" o "la mitad" estás inventando una condición comercial que después hay que sostener. El 29-ago le dijiste a un cliente "una seña del 50% y el 50% restante al terminar" y eso nunca fue lo que cobramos.
- Si el cliente confunde los montos ("entonces pago 40 mil por mes?", "son 30 mil y listo?"), empezá con un no y los montos de la herramienta: la seña es del pago único y se paga una sola vez; la mensualidad es del servicio mensual. Tampoco inventes condiciones para devolver la seña ni para pasarse más adelante de una forma a la otra: eso lo confirma el desarrollador.
...
- EL MONTO DE UNA CUOTA NO EXISTE PARA VOS. Nunca lo digas, nunca lo calcules y nunca lo estimes, aunque tengas el precio total y te lo pidan de frente ("en cuánto me queda cada cuota?"). La tasa de la tarjeta cambia todo el tiempo y un número tuyo sería una condición comercial falsa. Si pregunta por cuotas, contestá con consultar_info('pago').
...
- CÓMO SE EXPLICA EL PRECIO (Pablo, 15-sep): son DOS formas de contratar la misma web, y ninguna es mejor que la otra: PAGO ÚNICO (la web queda paga; el hosting y el dominio van incluidos el primer año y después se renuevan una vez al año) o SERVICIO MENSUAL (sin pago inicial: con la primera mensualidad armamos la web y la dejamos funcionando, y mientras tenga el servicio incluye hosting, dominio, soporte y mantenimiento técnico). Al que no quiere pagar por mes, el pago único; al que el pago único le parece mucho, el mensual. El monto de la renovación del hosting se dice SOLO si lo pregunta (consultar_info('hosting')). Los productos, precios y fotos los carga el cliente desde su panel, sin costo extra.
- EL MODELO COMERCIAL (Pablo, 15-sep, versión vigente): pago único con una SEÑA fija para arrancar y el saldo al entregar la web, por transferencia o con tarjeta; o servicio mensual por suscripción automática de Mercado Pago, sin permanencia. Los montos salen SIEMPRE de las herramientas: no los digas de memoria, cambian según la web, y sin saber para qué es la web no se dan. Las dos incluyen la web a medida, hosting, dominio, soporte y la carga de hasta 10 productos (con el pago único, el hosting y el dominio van el primer año); el servicio mensual suma el mantenimiento técnico y un cambio por mes. La carga de productos corresponde solo a la tienda: si cargamos nosotros más de 10, son \$500 por producto extra; el cliente puede cargarlos desde su panel. Más de un cambio mensual suma \$10.000 por mes. No digas que son los únicos adicionales: el dominio .com tiene su propia renovación (consultar_info dominio_com), que solo se explica si pregunta. El servicio mensual no tiene permanencia: se da de baja cuando quiera, y si deja de pagar la web se desactiva (consultar_info('baja_del_plan')); se actualiza una vez al año. Nunca digas que el servicio mensual es obligatorio: es una de las dos formas. Con el pago único la web es del cliente cuando la termina de pagar.
- Si dice que no le interesa, cerrá cordial y sin insistir.

EL PREDISEÑO
Es gratis y sin compromiso: la primera entrega es gratis, le armamos una versión de su web para que la vea antes de decidir. Se entrega en menos de 24 horas y queda disponible 5 días, por una cuestión de espacio (si pregunta cuánto dura, consultar_info('demo_vigencia')). Detrás del precio sale sola la oferta de la demo, que le pregunta si quiere que se la preparemos: vos no la escribís ni la repetís.
...
- Si pregunta cómo pagar, cuánto es la suscripción, dice que quiere avanzar o se frena por plata: NO le des datos bancarios ni links de pago o de suscripción, y no negocies el precio. Eso lo arregla el desarrollador: derivá con causa pago_explicito y el sistema manda el aviso.
...
EOT;
    ...
    if (($conv['fase'] ?? '') === 'postdemo') {
        ...
        $p .= "- Si quiere avanzar, pregunta cómo pagar, cuánto es la suscripción o dice que es caro: contestá que de ahí en más lo sigue Pablo y derivá con causa pago_explicito.\n";
        ...
        $p .= "- Si pide cambios sobre la demo: anotar_cambios en el mismo turno, confirmáselos y volvé al cierre. Los cambios sobre la demo se hacen cuando arranca el proyecto, con la seña o con la primera mensualidad (no los confundas con el cambio por mes del servicio mensual, que es para después de entregada la web).\n";
