...
caso('dar_precio(landing) → texto exacto, con lo que incluye y las dos formas, sin link (15-sep)',
    strpos((string)($r['texto'] ?? '') . "\n" . implode("\n", (array)($r['aparte'] ?? [])), '$20.000') !== false && strpos((string)($r['texto'] ?? '') . "\n" . implode("\n", (array)($r['aparte'] ?? [])), '$20.000') !== false
    && strpos($r['texto'], '1. Pago único de $180.000') !== false && strpos($r['texto'], '2. Servicio mensual de $20.000') !== false && strpos($r['texto'], 'presupuestos/') === false
...
caso('la primera llamada da la oferta y deja los tres pasos para un segundo mensaje (14-sep)',
    ...
    && stripos($r['texto'], 'Pago único de $290.000') !== false && strpos($r['texto'], 'presupuestos/') === false
 ...
 * existe la pregunta por la cantidad de productos. */
...
$pagoDeHerramienta = ['Se puede abonar por transferencia o con tarjeta hasta en 12 cuotas con interés. Para arrancar se deja una seña ($50.000 en landing o catálogo, $80.000 en turnos, institucional e inmobiliaria, $90.000 en ecommerce y plataforma de cursos) y el saldo al entregar la web.'];
...
caso('"que reserven solos" ya no cotiza turnos: va a sitio profesional',
    ($c['tipo'] ?? '') !== 'turnos');
...
caso('el modelo solo puede cotizar los cuatro tipos vigentes (2-sep)',
    $tipos === ['landing', 'ecommerce', 'inmobiliaria', 'elearning']);
caso('turnos, institucional, catálogo y LMS ya no están en la herramienta',
    !in_array('turnos', $tipos, true) && !in_array('institucional', $tipos, true)
    && !in_array('catalogo', $tipos, true) && !in_array('lms', $tipos, true));
...
caso('y le dice que la clasificación es una sola pregunta',
    strpos(wabot_agente_sistema($c, $cfg), 'VENDE PRODUCTOS O PRESTA UN SERVICIO') !== false);
...
$r = wabot_agente_ejecutar('consultar_info', ['clave' => 'mantenimiento'], $c, $cfg);
caso('ecommerce → el servicio mensual de $30.000, como una de las dos formas',
    strpos((string)($r['texto'] ?? '') . "\n" . implode("\n", (array)($r['aparte'] ?? [])), '$30.000') !== false && stripos($r['texto'], 'una de las dos formas') !== false);
...
caso('el playbook prohíbe ofrecer o mencionar la institucional, que se retiró (2-sep)',
    stripos($sistema, 'NUNCA ofrezcas ni menciones una web institucional') !== false);
caso('y aclara que una institución va a sitio profesional como cualquier otro',
    stripos($sistema, 'se cotiza como SITIO PROFESIONAL') !== false);
...
caso('el playbook explica el turno del precio: un solo llamado y dos mensajes (14-sep)',
    stripos($sistema, 'UN SOLO LLAMADO, DOS MENSAJES') !== false
...
caso('y aclara que un comercio nunca es un sitio profesional',
    strpos($sistema, 'NUNCA es un sitio profesional') !== false);
...
caso('el prompt describe el proceso con los tres pasos de Pablo, sin seña',
    stripos(wabot_agente_sistema($c, $cfg), 'Ese texto explica la demo gratis y las dos formas de contratar la web') !== false);
...
caso('y prohíbe mandar los datos bancarios y los links de pago o de suscripción',
    strpos(wabot_agente_sistema($cSinDemo, $cfg), 'NO los mandás nunca') !== false);
...
caso('y aunque diga que no quiere cobrar online, sigue siendo ecommerce (2-sep)',
    wabot_agente_desempate_pendiente('catalogo', 'quiero solo mostrar los productos y que me escriban por whatsapp', $cM, $cfg) === ['tipo' => 'ecommerce']);
...
caso('y la universidad que pide varias secciones también: el sitio profesional las lleva',
    empty($rUni['error']) && $cUni['tipo'] === 'landing');
...
/* Turnos está retirado: solo lo tiene una charla cotizada antes del 2-sep, con
 * el modelo viejo. La comparación se hace con los pagos únicos de entonces,
 * no con el primer pago del modelo nuevo, que es otra cosa (10-sep). */
caso('la comparación de turnos trae el precio de la landing y el ya cotizado, los dos del modelo de entonces',
    stripos((string)($rTurnos['texto'] ?? ''), 'landing') !== false
    && strpos((string)($rTurnos['texto'] ?? ''), wabot_precio_anterior_de('landing', $cfg)) !== false
    && strpos((string)($rTurnos['texto'] ?? ''), wabot_precio_anterior_de('turnos', $cfg)) !== false
    && empty($rTurnos['error']));
...
caso('y sigue con las dos formas y sus montos, sin link (15-sep)',
    strpos($pitchEst, 'gokywebs.com/presupuestos/') === false
    && strpos($pitchEst, '2. Servicio mensual de $20.000, sin pago inicial') !== false);
...
/* Y desde el 2-sep tampoco a una consultora: sin turnos no hay desempate que
 * hacer, todos los servicios son sitio profesional. */
$cCon = convNueva();
caso('un servicio suelto tampoco abre ninguna pregunta: se cotiza derecho',
    wabot_agente_desempate_pendiente('landing', 'Una consultora', $cCon, $cfg) === null);
$cTec = convNueva();
...
/* 10-sep: el plan mensual es OBLIGATORIO. Hasta ese día esta respuesta decía
 * "Es opcional. La web es tuya igual…", las dos frases que el modelo nuevo
...
caso('con el plan mensual en el último mensaje, contesta corto que no: es una de las dos formas (15-sep)',
    ($r = wabot_respuesta_obligatorio($cOpc, $cfg)) !== null && mb_strpos($r, 'No: el servicio mensual es una de las dos formas') === 0
...
caso('aunque el último mensaje sea el texto viejo que decía "opcional", sin cotización vieja la respuesta es la de las dos formas',
    ($rV = wabot_respuesta_obligatorio($cOpcTextoViejo, $cfg)) !== null && mb_strpos($rV, 'No: el servicio mensual es una de las dos formas') === 0);
...
caso('si pregunta si es obligatoria la DEMO, no le contesta sobre el plan',
    wabot_respuesta_obligatorio($cOpc, $cfg, 'es obligatorio hacer la demo?') === null);
