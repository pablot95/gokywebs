...
/* ---------- momento firma: ruta animada (avión -> pin) ---------- */
function initRouteSignature() {
  var svg = document.getElementById('routeMap');
  if (!svg) return;
  var path = svg.querySelector('.route-path');
  var nodes = svg.querySelectorAll('.route-node');
  ...
  nodes.forEach(function (n, i) {
    tl.to(n, { duration: .01, onStart: function () { n.classList.add('active'); } }, i === 0 ? 0 : '-=1.1');
  });
}
...
function initMap() {
  ...
  var coords = [-34.6037, -58.3816]; /* Obelisco / CABA (referencia de zona de cobertura) */
  var map = L.map(el, { scrollWheelZoom: false }).setView(coords, 11);
  L.tileLayer('https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png', {
    attribution: '&copy; OpenStreetMap &copy; CARTO', maxZoom: 19
  }).addTo(map);
  var icon = L.divIcon({
    className: '', iconSize: [26, 26],
    html: '<div style="width:26px;height:26px;border-radius:50%;background:#d4a35c;border:3px solid #0c1017;box-shadow:0 6px 16px rgba(0,0,0,.5)"></div>'
  });
  L.marker(coords, { icon: icon }).addTo(map).bindPopup('Nico Tours — zona de cobertura<br>CABA, Ezeiza y Aeroparque');
}
