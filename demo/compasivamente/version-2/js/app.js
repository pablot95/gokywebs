/* V2 "Presencia" — render + interacción. Usa data.js + core.js. */
...
/* Card (V2 editorial cálida, con tilt) */
function cardHTML(p, i) {
  ...
  return '<article class="p-card" data-animate data-tilt style="--i:'+(i||0)+'">'+
    ...
    '<div class="p-body"><div class="p-top"><span class="p-cat">'+esc(getCategoria(p.categoria))+'</span>'+precio+'</div>'+
    '<h3><a href="'+BASE+'producto/?id='+p.id+'">'+esc(p.nombre)+'</a></h3>'+
    (sinStock ? '<button class="btn btn-ghost btn-block" disabled>Sin stock</button>' :
    '<div class="p-actions"><button class="btn btn-cta btn-block" data-add="'+p.id+'">Agregar al carrito</button>'+
    '<button class="btn btn-ghost btn-block" data-buy="'+p.id+'">Comprar</button></div>')+
    '</div></article>';
}
function initTilt(root) {
  if (reduce || window.matchMedia('(hover: none)').matches) return;
  (root || document).querySelectorAll('[data-tilt]').forEach(el => {
    if (el.dataset.tiltOn) return; el.dataset.tiltOn = '1';
    el.addEventListener('mousemove', e => { const r = el.getBoundingClientRect(); const x = (e.clientX-r.left)/r.width-0.5, y = (e.clientY-r.top)/r.height-0.5; el.style.transform = 'perspective(700px) rotateY('+(x*5).toFixed(2)+'deg) rotateX('+(-y*5).toFixed(2)+'deg) translateY(-6px)'; });
...
function initIntention() {
  const el = document.getElementById('intentionTxt'); if (!el) return;
  const palabras = ['presencia', 'calma', 'aceptación', 'gratitud', 'respiro'];
  if (reduce) { el.textContent = palabras[0]; return; }
  let i = 0;
  const cycle = () => {
    el.style.opacity = '0'; el.style.transform = 'translateY(8px)';
    setTimeout(() => { i = (i+1) % palabras.length; el.textContent = palabras[i]; el.style.opacity = '1'; el.style.transform = 'none'; }, 450);
  };
