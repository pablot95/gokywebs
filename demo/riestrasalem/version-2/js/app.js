/* V2 "Claridad" — render + interacción. Usa data.js + core.js. */
...
/* ---------- Card de curso (V2 luminosa) ---------- */
function cardHTML(c, i) {
  const d = getDocente(c.docenteId), pf = precioFinal(c);
  const badge = c.nuevo ? '<span class="c-badge is-new">Nuevo</span>' : (c.destacado ? '<span class="c-badge">Más elegido</span>' : '');
  ...
  return '<article class="c-card" data-animate style="--i:' + (i||0) + '">' +
    ...
    '<div class="c-body">' +
    '<div class="c-chips"><span class="c-chip">' + esc(c.nivel) + '</span><span class="c-chip c-chip-mod">' + esc(c.modalidad) + '</span><span class="c-chip c-chip-dur">' + esc(c.duracion) + '</span></div>' +
    '<h3><a href="' + BASE + 'curso/?id=' + c.id + '">' + esc(c.titulo) + '</a></h3>' +
    '<div class="c-doc"><img src="' + IMG(d ? d.foto : '') + '" alt="" width="34" height="34" loading="lazy"><span>' + esc(d ? d.nombre : '') + '</span></div>' +
...
function initHome() {
  ...
  const ICONS = ['M3 6h18M3 12h18M3 18h12','M12 2l3 7h7l-5.5 4 2 7L12 16l-6.5 4 2-7L2 9h7z','M4 20a8 8 0 0116 0','M12 3a4 4 0 100 8 4 4 0 000-8zM4 21a8 8 0 0116 0','M8 3v4M16 3v4M4 9h16M5 5h14v16H5z'];
  ...
  if (areas) areas.innerHTML = CATEGORIAS.map((cat,i) => {
    ...
    return '<a class="area-card" href="catalogo.html?cat=' + cat.id + '"><span class="area-ico" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="' + ICONS[i % ICONS.length] + '"/></svg></span><h3>' + esc(cat.label) + '</h3><span class="area-count">' + n + ' curso' + (n!==1?'s':'') + '</span><span class="area-go">Ver cursos →</span></a>';
  }).join('');
...
function initCurso() {
  ...
  cont.innerHTML =
    ...
    '</div></aside></div>' +
    '<section class="cd-sec wrap"><h2 data-animate>Qué vas a lograr</h2><ul class="cd-result" data-animate>' + c.resultados.map(r => '<li>' + esc(r) + '</li>').join('') + '</ul></section>' +
